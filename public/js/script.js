const base_url = "https://localhost/controleFinanceiro"
...
class Dom {
    ...
    static val(selector, newValue) {
        ...
        if (!element) {
            ...
            return null;  // Ou retorne um valor padrão de sua escolha
        }
    ...
    static getDataAttributes(selector) {
        ...
        for (let i = 0; i < attributeList.length; i++) {
            ...
            if (attribute.name.startsWith('data-')) {
                const key = attribute.name.replace('data-', ''); // Remover o prefixo 'data-'
